...
export class Anchor extends XmlComponent {
    public constructor({
        ...
        const wrap = drawingOptions.floating?.wrap;
        if (wrap !== undefined) {
            const wrapMargins = wrap.margins ?? drawingOptions.floating?.margins;
            switch (wrap.type) {
                case TextWrappingType.SQUARE:
                    this.root.push(new WrapSquare(wrap, wrapMargins));
                    break;
                case TextWrappingType.TIGHT:
                    this.root.push(new WrapTight(wrapMargins));
                    break;
                case TextWrappingType.TOP_AND_BOTTOM:
                    this.root.push(new WrapTopAndBottom(wrapMargins));
                    break;
