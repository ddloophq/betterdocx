...
Math requires an understanding of [Sections](sections) and [Paragraphs](paragraph).
...
![Math Example](math-example.png "Math Example")

## Math Components

`MathComponents` are the unit sized building blocks of an equation in `docx`. A `MathComponent` takes in more nested `MathComponents` until you reach `MathRun`, which has no children. `MathRun` is similar to a [TextRun](text).
...
A `MathComponent` for the `∫` symbol. Like `MathSum`, it takes optional `superScript` and/or `subScript` arguments for the integration bounds. The bounds are rendered beside the integral sign.

```ts
new MathIntegral({
    children: [new MathRun("x dx")],
}),
```
...
`MathPreSubSuperScripts` place the scripts to the left of the base instead of the right. Both `subScript` and `superScript` are required.

```ts
new MathPreSubSuperScript({
    children: [new MathRun("x")],
    superScript: [new MathRun("14")],
    subScript: [new MathRun("6")],
}),
```
