...
export const universalMeasureValue = (val: UniversalMeasure): UniversalMeasure => {
    ...
    if (isNaN(amount)) {
        throw new Error(`Invalid value '${val}' specified. Expected a valid universal measure.`);
    }
    return `${amount}${unit}` as UniversalMeasure;
};
...
export const percentageValue = (val: Percentage): Percentage => {
    const percent = Number(val.substring(0, val.length - 1));
    if (isNaN(percent)) {
        throw new Error(`Invalid value '${val}' specified. Expected a valid percentage.`);
    }
    return `${percent}%`;
};
