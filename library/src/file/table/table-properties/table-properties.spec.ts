...
import { OverlapType } from "./table-float-properties";
...
describe("TableProperties", () => {
    describe("#constructor", () => {
        ...
        it("writes table overlap beside floating-position properties", () => {
            const tp = new TableProperties({
                float: {
            ...
            expect(new Formatter().format(tp)).to.deep.equal({
                "w:tblPr": [
        ...
        it("does not emit an empty floating-position element for overlap alone", () => {
            const tp = new TableProperties({ float: { overlap: OverlapType.OVERLAP } });

            expect(new Formatter().format(tp)).to.deep.equal({
                "w:tblPr": [{ "w:tblOverlap": { _attr: { "w:val": "overlap" } } }],
            });
        });
    });
    ...
    describe("#tableStyleOptions", () => {
        it("adds table look with header row and first column enabled", () => {
            const tp = new TableProperties({
                tableStyleOptions: {
                    headerRow: true,
                    firstColumn: true,
