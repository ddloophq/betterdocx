...
describe("ParagraphProperties", () => {
    describe("#constructor()", () => {
        ...
        it.each([
            ["pageBreakBefore", { pageBreakBefore: false }, "w:pageBreakBefore", false],
            ["wordWrap", { wordWrap: false }, "w:wordWrap", 1],
            ["overflowPunctuation", { overflowPunctuation: false }, "w:overflowPunct", false],
        ] as const)("should preserve an explicit false %s value", (_, options, key, value) => {
            const tree = new Formatter().format(new ParagraphProperties(options));

            expect(tree).to.deep.equal({
                "w:pPr": [{ [key]: { _attr: { "w:val": value } } }],
            });
        });
