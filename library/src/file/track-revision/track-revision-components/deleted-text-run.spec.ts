...
describe("DeletedTextRun", () => {
    ...
    it("renders current-section fields explicitly and equal-looking strings literally", () => {
        const tree = new Formatter().format(
            new DeletedTextRun({
                children: [PageNumber.CURRENT_SECTION, "SECTION"],
                id: 0,
                date: "123",
                author: "Author",
            }),
        );
        const run = tree["w:del"][1]["w:r"];

        expect(run[1]).to.deep.equal({
            "w:delInstrText": [{ _attr: { "xml:space": "preserve" } }, "SECTION"],
        });
        expect(run[4]).to.deep.equal({
            "w:delText": [{ _attr: { "xml:space": "preserve" } }, "SECTION"],
        });
    });
});
