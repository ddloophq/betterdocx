...
describe("Run", () => {
    ...
    describe("#verticalAlignReset()", () => {
        it("emits baseline for an explicit false script option", () => {
            const tree = new Formatter().format(new Run({ subScript: false }));

            expect(tree).to.deep.equal({
                "w:r": [{ "w:rPr": [{ "w:vertAlign": { _attr: { "w:val": "baseline" } } }] }],
            });
        });

        it("rejects contradictory script options", () => {
            expect(() => new Run({ subScript: true, superScript: true })).to.throw(
                "A run cannot be both subscript and superscript",
            );
        });
    });
    ...
    describe("#literalPageNumberNames", () => {
        it.each(["CURRENT", "TOTAL_PAGES", "TOTAL_PAGES_IN_SECTION", "SECTION"])(
            "renders %s as literal text",
            (text) => {
                const tree = new Formatter().format(new Run({ children: [text] }));

                expect(tree).to.deep.equal({
                    "w:r": [
                        {
                            "w:t": [{ _attr: { "xml:space": "preserve" } }, text],
                        },
                    ],
                });
            },
    ...
    describe("#explicitFalsyFormatting", () => {
        it("emits explicit false and meaningful zero values", () => {
            ...
            expect(tree).to.deep.equal({
                "w:r": [
                    {
                        "w:rPr": [
                            { "w:vanish": { _attr: { "w:val": false } } },
                            { "w:spacing": { _attr: { "w:val": 0 } } },
                            { "w:kern": { _attr: { "w:val": 0 } } },
                            { "w:position": { _attr: { "w:val": 0 } } },
                            { "w:szCs": { _attr: { "w:val": 0 } } },
                            { "w:specVanish": { _attr: { "w:val": false } } },
                            { "w:oMath": { _attr: { "w:val": false } } },
                        ],
                    },
                ],
            });
