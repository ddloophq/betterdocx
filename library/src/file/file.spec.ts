...
describe("File", () => {
    ...
    describe("#createFootnote", () => {
        ...
        it.each(["0", "-1", "1.5", "1junk", "01", "NaN", "Infinity", "foo"])(
            "should reject invalid footnote id %s",
            (id) => {
                expect(
                    () =>
                        new File({
                            footnotes: {
                                [id]: { children: [new Paragraph("invalid")] },
                            },
                        }),
                ).toThrow(/Invalid footnote id/);
            },
        );

        it("should reject footnote ids outside JavaScript's safe integer range", () => {
            expect(
                () =>
                    new File({
                        footnotes: {
                            9007199254740992: { children: [new Paragraph("invalid")] },
                        },
                    }),
