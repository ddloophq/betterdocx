...
// Runs carry a reference to their live run element for in-place patching;
// drop it here so the assertions can compare against plain literals.
const withoutRunElements = (
    paragraphs: ReturnType<typeof findLocationOfText>,
): readonly unknown[] =>
    paragraphs.map((paragraph) => ({
        ...paragraph,
        runs: paragraph.runs.map(({ element: _, ...run }) => run),
    }));
...
describe("traverser", () => {
    describe("findLocationOfText", () => {
        it("should find the location of text", () => {
            ...
            expect(output[0].runs[0].element.name).to.equal("w:r");
            expect(withoutRunElements(output)).to.deep.equal([
                {
                    index: 1,
