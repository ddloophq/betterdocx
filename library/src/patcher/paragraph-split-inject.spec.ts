...
describe("paragraph-split-inject", () => {
    describe("findRunElementIndexWithToken", () => {
        it("should find the index of a run element with a token", () => {
            ...
            const output = findRunElementIndexWithToken(paragraphElement, "hello");
            expect(output.index).to.equal(0);
            expect(output.parent).to.equal(paragraphElement);
        });

        it("should find a run element nested inside an inline wrapper", () => {
            ...
            const output = findRunElementIndexWithToken(paragraphElement, "hello");
            expect(output.index).to.equal(0);
            expect(output.parent).to.equal(hyperlinkElement);
        });
