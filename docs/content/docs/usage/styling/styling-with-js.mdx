...
`docx` files contain a styles section separate from the main content, much like how HTML includes CSS files. Unlike CSS, `docx` distinguishes between styles meant for tables (which show up in the table formatting toolbar), styles for lists (which show up under bullets and numbering), and styles for runs and paragraphs, which show up as dropdowns offering standard styles, like "Heading 1", "Caption", or any custom styles defined in that document. {/* TODO: add pictures of the panes */}

Of these, only **paragraph styles** and **character styles** can be defined through the JS API (via `styles.paragraphStyles` and `styles.characterStyles`). Table styles are not exposed in JS — to use them, supply a complete styles definition from an external XML file as described in [Styling with XML](styling-with-xml).
...
**Note**: The library always generates built-in `Title`, `Heading1`–`Heading6`, `ListParagraph` and other default styles for you (see [Default style overrides](#default-style-overrides) below). Defining a paragraph style with the same `id` (like `Heading2` above) redefines that style wholesale; if you just want to tweak a built-in style, prefer `styles.default`.

#### Character styles

Character styles carry run formatting only. Define them with `styles.characterStyles` and apply them through a run's `style` option:
    ...
    sections: [
        {
            children: [
                new Paragraph({
                    children: [
                        new TextRun("Plain text, followed by "),
                        new TextRun({
                            text: "text using the character style",
...
Unless you say otherwise, character styles are emitted with `uiPriority: 99` and `unhideWhenUsed: true`, matching how Word treats custom character styles; pass your own values to override this.

#### Style metadata

Besides `run` and `paragraph` formatting, both paragraph and character styles accept metadata fields that control inheritance and how the style behaves in Word's styles pane:

- `name`: Human-friendly name shown in the UI
- `basedOn`: `id` of the style to inherit from (must be a style of the same type)
- `next`: `id` of the style applied to the following paragraph when you press Enter
- `link`: `id` of a linked style, used to pair a paragraph style with its character-style counterpart
- `uiPriority`: Sorting position of the style within the styles pane
- `semiHidden`: Hide the style from the UI until it is used
- `unhideWhenUsed`: Reveal the style in the UI once it appears in the document
- `quickFormat`: Show the style in the quick-format style gallery

#### Imported styles

`styles.importedStyles` accepts pre-built style components — `StyleForParagraph` / `StyleForCharacter` instances or `ImportedXmlComponent`s parsed from existing XML — and injects them into the styles part as-is. **Caveat**: supplying `importedStyles` replaces the library's built-in default styles (document defaults, headings, etc.) entirely, so include your own equivalents if you need them.

### Document defaults

Setting document defaults acts like a `*` rule in CSS: it applies to every paragraph and run in the document, but at a low priority level. Other styles affecting this property will override these defaults. Set them via `styles.default.document`:
...
The library ships built-in styles that back the `heading`/`title` paragraph options, bullets/numbering, hyperlinks and footnotes. You can customize each of them under `styles.default`:

- Paragraph styles: `title`, `heading1` … `heading6`, `strong`, `listParagraph`, `footnoteText`
- Character styles: `hyperlink`, `footnoteReference`, `footnoteTextChar`
- Plus `document` for the document defaults shown above
...
Your overrides are merged into the built-in defaults one level deep within `run` and `paragraph`: the `heading1` override above keeps the built-in Heading 1 color (`2E74B5`) and size while adding bold. Note that any individual property you set replaces the built-in value for that property wholesale — including properties that are themselves objects, such as `underline`.
