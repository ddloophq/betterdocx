...
The first constructor argument is the alias of the structured document tag that wraps the table of contents. Word shows it in the header of the content control when the table is selected. It defaults to `"Table of Contents"` if omitted; in the example above it is set to `"Summary"`.

```ts
new TableOfContents(); // alias defaults to "Table of Contents"
new TableOfContents("Summary", { hyperlink: true });
```
...
The `stylesWithLevels` option takes an array of `StyleLevel` objects, each constructed with a style name and the table of contents level it maps to: `new StyleLevel(styleName, level)`.
