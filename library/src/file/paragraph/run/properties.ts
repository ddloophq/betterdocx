...
} from "@file/track-revision/track-revision";
import {
    HpsMeasureElement,
...
import { PositiveUniversalMeasure, UniversalMeasure, signedHpsMeasureValue } from "@util/values";

import { EmphasisMark, EmphasisMarkType } from "./emphasis-mark";
import { CharacterSpacing, Color, Highlight, HighlightComplexScript } from "./formatting";
import { ILanguageOptions, createLanguageComponent } from "./language";
import { IFontAttributesProperties, RunFonts } from "./run-fonts";
import { Baseline, SubScript, SuperScript } from "./script";
import { IUnderlineOptions, Underline } from "./underline";
...
export type IRunStylePropertiesOptions = {
    ...
    readonly underline?: IUnderlineOptions;
    ...
    readonly position?: number | UniversalMeasure;
...
class Position extends XmlComponent {
    public constructor(value: number | UniversalMeasure) {
        super("w:position");
        this.root.push(new Attributes({ val: signedHpsMeasureValue(value) }));
    }
}
...
export class RunProperties extends IgnoreIfEmptyXmlComponent {
    public constructor(options?: IRunPropertiesOptions) {
        ...
        // Only having both *enabled* is contradictory; explicit false values
        // must still be emitted so they can override inherited styles.
        if (options.smallCaps !== undefined) {
            this.push(new OnOffElement("w:smallCaps", options.smallCaps));
        }

        if (options.allCaps !== undefined && !(options.smallCaps && options.allCaps)) {
        ...
        if (options.characterSpacing !== undefined) {
        ...
        if (options.position !== undefined) {
            this.push(new Position(options.position));
        }
        ...
        if (options.underline) {
            this.push(new Underline(options.underline));
        }
        ...
        if (options.subScript && options.superScript) {
            throw new Error("A run cannot be both subscript and superscript");
        }
        ...
        } else if (options.subScript !== undefined || options.superScript !== undefined) {
            this.push(new Baseline());
        }
        ...
        if (options.specVanish !== undefined) {
